...
rom[0]=0x42; rom[0x100:0x104]=bytes([0x00,0xC3,0x50,0x01]); rom[0x104:0x134]=logo
rom[0x134:0x13F]=b'MBC5SGDKV7\0'; rom[0x147]=0x1B; rom[0x148]=0x08; rom[0x149]=0x03
class A:
    ...
    def finish(self):
        for i,n in self.rel:
            ...
            self.b[i]=d&255
        return bytes(self.b)
for bank in range(1,512):
    off=bank*0x4000; rom[off:off+5]=bytes([0x3E,bank&0xFF,0x06,(bank>>8)&1,0xC9])
m=A(0x150); m.e(0x31); m.u16(0xFFFE); m.e(0x21); m.u16(0xC100); m.e(0x06,0x00,0x0E,0x01) # HL output, B=high, C=low
m.label('banks')
m.e(0x79); m.sta(0x2000); m.e(0x78); m.sta(0x3000); m.call(0x4000)
m.e(0x22,0x78,0x22) # output low A; output high B
m.e(0x0C); m.jr(0x20,'banks') # INC C; JR NZ => same high bit
m.e(0x04,0x78,0xFE,0x02); m.jr(0x20,'banks') # INC B; CP 2; JR NZ
m.lda(0); m.sta(0x2000); m.lda(0); m.sta(0x3000); m.lda_mem(0x4000); m.sta(0xC500)
m.lda(0x0A); m.sta(0x0000)
for bank,val in enumerate((0xD0,0xD1,0xD2,0xD3)):
    m.lda(bank); m.sta(0x4000); m.lda(val); m.sta(0xA000)
for bank in range(4):
    m.lda(bank); m.sta(0x4000); m.lda_mem(0xA000); m.sta(0xC510+bank)
m.lda(0); m.sta(0x0000); m.lda_mem(0xA000); m.sta(0xC514)
...
base=Path(__file__).parent; (base/'mbc5test.gb').write_bytes(rom)
(base/'mbc5test.annotations').write_text('function 00:0150 Main\n'+''.join(f'function {b:03x}:4000 Bank{b:03d}\n' for b in range(1,512)))
print(f'wrote mbc5test.gb {len(rom)} bytes main={len(code)} annotated_banks=511')

...
static inline void gb_pack_flags(GBContext *ctx) {
    ctx->f=(uint8_t)((ctx->f_z ? 0x80u : 0u) |
                     (ctx->f_n ? 0x40u : 0u) |
                     (ctx->f_h ? 0x20u : 0u) |
                     (ctx->f_c ? 0x10u : 0u));
}
static inline void gb_unpack_flags(GBContext *ctx) {
    ctx->f_z=(uint8_t)((ctx->f & 0x80u) != 0u);
    ctx->f_n=(uint8_t)((ctx->f & 0x40u) != 0u);
    ctx->f_h=(uint8_t)((ctx->f & 0x20u) != 0u);
    ctx->f_c=(uint8_t)((ctx->f & 0x10u) != 0u);
}
...
static inline uint8_t gbrt_timed_bus_read8(GBContext *ctx, uint16_t addr, uint8_t leading_cycles) {
    gb_tick(ctx, leading_cycles);
    const uint8_t value=gb_read8(ctx, addr);
    gb_tick(ctx, 1u);
    return value;
}
static inline void gbrt_timed_bus_rmw_write8(GBContext *ctx, uint16_t addr, uint8_t value) {
